...
async def fast_download(
    client: Client, message, file_path: str, progress: Optional[Callable] = None
):
    """
    ...
    if file_size == 0 or file_size < CHUNK_SIZE:
        ...
        return await client.download_media(
            message, file_name=file_path, progress=progress
        )
    ...
    try:
        ...
        logger.warning(
            f"Fast download failed ({e}). Falling back to standard download."
        )
...
async def fast_upload(
    client: Client, file_path: str, progress: Optional[Callable] = None
):
    """
                    ...
                    if is_big:
                        ...
                        await client.invoke(
                            functions.upload.SaveFilePart(
                                file_id=file_id, file_part=part_num, bytes=chunk
                            )
                        )
